...
- 2026-06-16: PROD SCHEMA GAP FIXED. Discovered prod was on migration 009 (builders applied 010/011 to munin_test only). Backed up (munin-pre-schema1011-20260616-102148.dump) + applied 010+011 to prod via MUNIN_PG_DB=munin migrate.sh (2 applied, 9 skipped). Verified: valid_from/valid_to columns present, match_thoughts filters valid_to, 4885 rows preserved, 0 null valid_from. Prod now consistent with code.
- 2026-06-16: final-adversarial CYCLE 2 (verification of cycle-1 diff). Mostly VERIFIED-OK. 1 latent must-fix: MMR called k=match_limit while candidates capped at rerank_top_n=25 → silent short-return if caller limit>25 (never hit at default limit=10, but contract violation). + 2 cheap should-fix (eval liveness add valid_to; F1 re-flag was a misread — fixer DID measure on Gemma). CYCLE 3 fix (orchestrator, commit 73584ae): k=min(match_limit,len(candidates)); eval _check_live_thoughts adds valid_to IS NULL. pytest 312 passed, ruff+mypy clean. Eval on consistent prod holds: 0.70/0.90/0.95/0.792. CONVERGED — remaining items are nits. Gate CLEAN.
- RUN PROGRESS: [x] P2-1 [x] P2-2 [x] P2-3 [x] P2-4 [x] P2-review [x] P2-fix [x] P3-1 [x] P3-2 [x] P3-3 [x] P3-review [x] P3-fix [x] final-adversarial(3 cycles, converged) [ ] merge [ ] relaunch [ ] re-score+proof
